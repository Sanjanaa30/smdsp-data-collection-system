...
def plot_post_data(df, title):
    """Helper function to plot post data by hour"""
    ...
    for board in df["board_name"].unique():
        ...
        plt.plot(board_data["hour_only"], board_data["post_count"], marker='o', label=board, linewidth=2)
    ...
    # Show the plot (block=False to allow multiple plots)
    plt.tight_layout()
    plt.show(block=False)
    plt.pause(0.1)  # Small pause to ensure plot renders


def process_data_for_plot(query, columns, title):
    """Fetches data, processes it, and plots the graph"""
    ...
    if data:
        ...
        # Plot the data
        plot_post_data(df, title)
    else:
        logger.warning(f"No data available for query: {query}")


def main():
    """Main function to execute queries and plot data"""
    ...
    if hourly_data:
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(df["hour_only"], df["post_count"], marker='o', linewidth=2, color='blue')
        plt.xlabel("Hour of Day (0-23)")
    ...
    # Keep plots open
    logger.info("Displaying plots. Close plot windows to exit.")
    plt.show()
