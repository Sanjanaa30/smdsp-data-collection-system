...
-- Add Perspective API toxicity score columns to posts table
...
CREATE TABLE IF NOT EXISTS post_toxicity (
  ...
  PRIMARY KEY (board_name, post_no)  -- one row per post; we UPSERT on this
);

CREATE INDEX IF NOT EXISTS board_recent_idx
  ON post_toxicity (board_name, scored_at DESC);
